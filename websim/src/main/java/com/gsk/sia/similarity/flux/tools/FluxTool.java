package com.gsk.sia.similarity.flux.tools;
...
public class FluxTool implements ApplicationTool, Recyclable {
	...
	public List<User> getUsers() {
		try {
			...
			// Exclude the anonymous account
			List<User> tmp_list = (List<User>) security.getUserManager().retrieveList(criteria);
			List<User> final_list = new ArrayList<User>();
			for ( User u : tmp_list )
				if ( !u.getName().toLowerCase().contentEquals("anon") )
					final_list.add(u);
	 ...
	 * UserList.vm template.
	 */
	public List<InsightUser> getInsightUsers() {
		return InsightUser.getAllUsers();
	}
